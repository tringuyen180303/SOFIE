...
public:
  ...
  ROperator_BatchNormalization(
      float epsilon, float momentum, std::size_t training_mode,
    ...
    if (std::is_same<T, float>::value) {
      ...
      throw std::runtime_error("TMVA SOFIE Encountered unsupported type "
                               "parsing a BatchNormalization operator");
    }
  }
  ...
  ShapeInference(std::vector<std::vector<size_t>> input) override {
    if (input.size() != 5) {
      throw std::runtime_error("TMVA SOFIE BatchNormalization Op Shape "
                               "inference need 5 input tensors");
    }
    for (size_t i = 0; i < input.size(); i++) {
      if (input[i].size() != 4) {
        throw std::runtime_error(
            "TMVA SOFIE BatchNormalization Op Shape inference only accept "
  ...
  void Initialize(RModel &model) override {
    if (!model.CheckIfTensorAlreadyExist(fNX)) {
      throw std::runtime_error(
          "TMVA SOFIE BatchNormalization op Input Tensor " + fNX +
          " fnx is not found in model");
    }
    if (!model.CheckIfTensorAlreadyExist(fNScale)) {
      throw std::runtime_error(
          "TMVA SOFIE BatchNormalization op Input Tensor " + fNScale +
          " fns is not found in model");
    }
    if (!model.CheckIfTensorAlreadyExist(fNB)) {
      throw std::runtime_error(
          "TMVA SOFIE BatchNormalization op Input Tensor " + fNB +
          " fnb is not found in model");
    }
    if (!model.CheckIfTensorAlreadyExist(fNMean)) {
      throw std::runtime_error(
          "TMVA SOFIE BatchNormalization op Input Tensor " + fNMean +
          " fnm is not found in model");
    }
    if (!model.CheckIfTensorAlreadyExist(fNVar)) {
      throw std::runtime_error(
          "TMVA SOFIE BatchNormalization op Input Tensor " + fNVar +
    ...
    if (fShapeX.size() < 2 || fShapeX.size() > 4) {
      throw std::runtime_error(
          "TMVA SOFIE BatchNormalization Op input tensor " + fNX +
          " fnx has wrong shape : " + ConvertShapeToString(fShapeX));
    }
    ...
    if (fShapeB.size() == 1) {
      ...
      if (fType == "float") {
        ...
        for (ch = 0; ch < channels; ch++) {
          for (h = 0; h < height; h++) {
            for (w = 0; w < width; w++) {
              new_bias[bs * channels * height * width + ch * height * width +
                       h * width + w] = original_bias[ch];
              new_scale[bs * channels * height * width + ch * height * width +
                        h * width + w] = original_scale[ch];
              new_mean[bs * channels * height * width + ch * height * width +
                       h * width + w] = original_mean[ch];
              new_var[bs * channels * height * width + ch * height * width +
                      h * width + w] = original_var[ch];
            }
          }
        }
        size_t Batchoffset = channels * height * width;
        for (bs = 1; bs < batchSize; bs++) {
          std::copy(new_bias, new_bias + Batchoffset,
                    new_bias + (bs * Batchoffset));
          std::copy(new_scale, new_scale + Batchoffset,
                    new_scale + (bs * Batchoffset));
          std::copy(new_mean, new_mean + Batchoffset,
                    new_mean + (bs * Batchoffset));
          std::copy(new_var, new_var + Batchoffset,
                    new_var + (bs * Batchoffset));
        }
        ...
        std::vector<size_t> new_bias_shape = {batchSize, channels, height,
                                              width};
        std::shared_ptr<void> new_bias_ptr(new_bias,
                                           std::default_delete<float[]>());
        std::shared_ptr<void> new_scale_ptr(new_scale,
                                            std::default_delete<float[]>());
        std::shared_ptr<void> new_mean_ptr(new_mean,
                                           std::default_delete<float[]>());
        std::shared_ptr<void> new_var_ptr(new_var,
                                          std::default_delete<float[]>());
        model.UpdateInitializedTensor(fNB, model.GetTensorType(fNB),
                                      new_bias_shape, new_bias_ptr);
        model.UpdateInitializedTensor(fNScale, model.GetTensorType(fNScale),
                                      new_bias_shape, new_scale_ptr);
        model.UpdateInitializedTensor(fNMean, model.GetTensorType(fNMean),
                                      new_bias_shape, new_mean_ptr);
        model.UpdateInitializedTensor(fNVar, model.GetTensorType(fNVar),
                                      new_bias_shape, new_var_ptr);
        fShapeB = model.GetTensorShape(fNB);
        fShapeScale = model.GetTensorShape(fNScale);
        fShapeMean = model.GetTensorShape(fNMean);
        fShapeVar = model.GetTensorShape(fNVar);
  ...
  std::string Generate(std::string OpName) override {
    OpName = "op_" + OpName;
    if (fShapeX.empty()) {
      throw std::runtime_error("TMVA SOFIE Batch Normalization called to "
                               "Generate without being initialized first");
    }
    ...
    //// copy X into Y
    out << "\n\n//---- BatchNorm\n";
    out << SP << "constexpr int " << OpName
        << "_N =" << batchSize * channels * height * width << ";\n";
    out << SP << "constexpr int " << OpName << "_incx = 1;\n";
    out << SP << "constexpr int " << OpName << "_incy = 1;\n";
    out << SP << "BLAS::scopy_(&" << OpName << "_N, " << "tensor_" << fNX
        << ", &" << OpName << "_incx," << "tensor_" << fNY << ", &" << OpName
        << "_incy);\n\n";

    //// blas saxpy (Y = -Bmean + Y)
    out << SP << "float " << OpName << "_alpha = -1;\n";
    out << SP << "BLAS::saxpy_(&" << OpName << "_N, &" << OpName << "_alpha, "
        << "tensor_" << fNMean << ", &" << OpName << "_incx,"
        << "tensor_" << fNY << ", &" << OpName << "_incy);\n\n ";

    //// Y *= scale*var
    out << SP << "for (size_t i = 0; i < " << n << "; i++) {\n";
    // scale tensor contains already the var
    out << SP << SP << "tensor_" << fNY << "[i] *= tensor_" << fNScale
        << "[i]; \n";
    out << SP << "}\n";
    ...
    out << SP << "BLAS::saxpy_(&" << OpName << "_N, &" << OpName << "_alpha, "
        << "tensor_" << fNB << ", &" << OpName << "_incx, "
        << "tensor_" << fNY << ", &" << OpName << "_incy);\n\n";

    if (fActivation == EActivationType::RELU) {
      out << SP << "for (int id = 0; id < " << ConvertShapeToLength(fShapeY)
          << " ; id++){\n";
      out << SP << SP << "tensor_" << fNY << "[id] = ((tensor_" << fNY
          << "[id] > 0 )? tensor_" << fNY << "[id] : 0);\n";
      out << SP << "}\n";
    }
    return out.str();
  }
  ...
  std::string Generate_GPU_Kernel_ALPAKA(std::string opName) override {
    ...
    out << SP
        << "ALPAKA_FN_ACC void operator()(TAcc const & acc, T const* "
           "__restrict__ data, T const* __restrict__ mean, T const* "
           "__restrict__ scale, T const* __restrict__ bias, T* __restrict__ "
           "out, std::size_t numElements) const {\n";
    out << SP << SP
        << "const auto idx = alpaka::getIdx<alpaka::Grid, "
           "alpaka::Threads>(acc)[0];\n";
    out << SP << SP << "if(idx < numElements) {\n";
    out << SP << SP << SP
        << "out[idx] = (data[idx] - mean[idx]) * scale[idx] + bias[idx];\n";
    if (fActivation == EActivationType::RELU) {
      out << SP << SP << SP
          << "out[idx] = (out[idx] > static_cast<T>(0)) ? out[idx] : "
             "static_cast<T>(0);\n";
    }
    out << SP << SP << "}\n";
    out << SP << "}\n";
  ...
  Generate_GPU_Kernel_Definitions_ALPAKA(std::string opName) override {
    opName = "op_" + opName;
    return SP + "BatchNormKernel_" + opName + " batchNormKernel_" + opName +
           ";\n";
  }
  ...
  std::string Generate_GPU_ALPAKA(std::string OpName) override {
    ...
    out << SP << "auto const workDiv_" << fNX
        ...
        << ", devAcc, batchNormKernel_" << op_name
  ...
  std::vector<std::string> GetBlasRoutines() override {
    return {std::string("Copy"), std::string("Axpy")};
  }
};
